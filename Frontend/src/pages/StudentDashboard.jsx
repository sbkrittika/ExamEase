...
import { Calendar, Clock, MapPin, UserCheck, LogOut } from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  return <div className="min-h-screen bg-slate-50 p-6 sm:p-10">
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-3">{data.schedule.map((exam) => <article key={`${exam.exam_id}-${exam.room_number}`} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm"><div className="flex flex-wrap justify-between gap-3"><h2 className="font-semibold text-slate-900 flex gap-2 items-center"><Calendar size={17} className="text-blue-600" />{new Date(`${exam.exam_date}T00:00:00`).toLocaleDateString()}</h2></div><div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-slate-600"><span className="flex gap-2 items-center"><Clock size={16} />{exam.start_time} – {exam.end_time}</span><span className="flex gap-2 items-center"><MapPin size={16} />Room {exam.room_number}</span><span className="flex gap-2 items-center"><UserCheck size={16} />{exam.invigilator}</span></div></article>)}</div>
    </div>
  </div>;
}
