...
const listStudents = async (req, res) => {
    ...
    } catch (err) { asError(res, "Failed to fetch students.", err); }
};

const saveStudent = async (req, res) => {
    const { student_id, student_name, semester, course_code } = req.body || {};
    if (!student_id || !student_name || !semester || !course_code) {
        return res.status(400).json({ success: false, message: "Student ID, name, semester and course are required." });
    }
    try {
        await db.promise().query(
            ...
            [String(student_id).trim(), String(student_name).trim(), Number(semester), String(course_code).trim()]
        );
        res.status(201).json({ success: true, message: "Student saved successfully." });
    } catch (err) { asError(res, "Failed to save student.", err); }
};

const deleteStudent = async (req, res) => {
    try {
        await db.promise().query("DELETE FROM students WHERE student_id = ?", [req.params.id]);
        res.json({ success: true, message: "Student deleted successfully." });
    } catch (err) { asError(res, "Failed to delete student.", err); }
};
...
const listRooms = async (req, res) => {
    try {
        const [rows] = await db.promise().query("SELECT room_id, room_number, building, capacity, status FROM rooms ORDER BY building, room_number");
        res.json({ success: true, rooms: rows });
    } catch (err) { asError(res, "Failed to fetch rooms.", err); }
};

const saveRoom = async (req, res) => {
    const { room_number, building, capacity, status } = req.body || {};
    if (!room_number || !capacity || Number(capacity) < 1) {
    ...
    try {
        const [result] = await db.promise().query(
            ...
            [String(room_number).trim(), building ? String(building).trim() : null, Number(capacity), status === "Unavailable" ? "Unavailable" : "Available"]
        );
        res.status(201).json({ success: true, room_id: result.insertId || null, message: "Room saved successfully." });
    } catch (err) { asError(res, "Failed to save room.", err); }
};

const deleteRoom = async (req, res) => {
    try {
        await db.promise().query("DELETE FROM rooms WHERE room_id = ?", [req.params.id]);
        res.json({ success: true, message: "Room deleted successfully." });
    } catch (err) { asError(res, "Failed to delete room.", err); }
};
...
const dashboard = async (req, res) => {
    try {
        ...
        const [[rooms]] = await db.promise().query("SELECT COUNT(*) AS count FROM rooms WHERE status='Available'");
        ...
        res.json({ success: true, stats: { students: students.count, courses: courses.count, faculty: faculty.count, rooms: rooms.count }, upcoming });
    } catch (err) { asError(res, "Failed to fetch dashboard data.", err); }
};
...
const mySchedule = async (req, res) => {
    ...
    } catch (err) { asError(res, "Failed to fetch your schedule.", err); }
};
    ...
    try {
        const [rows] = await db.promise().query(
            `SELECT ia.assignment_id,ia.exam_id,ia.room_id,ia.faculty_id,e.exam_date,e.start_time,
             ...
             FROM invigilator_assignments ia JOIN exams e ON e.exam_id=ia.exam_id
             JOIN rooms r ON r.room_id=ia.room_id JOIN users u ON u.user_id=ia.faculty_id
             ORDER BY e.exam_date,e.start_time,r.room_number`
        );
        res.json({ success: true, assignments: rows });
    } catch (err) { asError(res, "Failed to fetch invigilation assignments.", err); }
};
...
const assignInvigilator = async (req, res) => {
    ...
    try {
        await db.promise().query(
            ...
            [exam_id, room_id, faculty_id]
        );
        res.status(201).json({ success: true, message: "Invigilator assigned successfully." });
    } catch (err) { asError(res, "Failed to assign invigilator.", err); }
};

const removeAssignment = async (req, res) => {
    try {
        await db.promise().query("DELETE FROM invigilator_assignments WHERE assignment_id=?", [req.params.id]);
        res.json({ success: true, message: "Assignment removed successfully." });
    } catch (err) { asError(res, "Failed to remove assignment.", err); }
};
